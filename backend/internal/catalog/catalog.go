...
// games) behind one search/details interface. IGDB participates only
// when Twitch app credentials are configured.
type Service struct {
	steam *steam.Client
	igdb  *igdb.Client
}

func New(steamClient *steam.Client, igdbClient *igdb.Client) *Service {
	return &Service{steam: steamClient, igdb: igdbClient}
}
